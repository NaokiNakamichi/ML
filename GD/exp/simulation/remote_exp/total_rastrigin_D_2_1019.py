...
if __name__ == "__main__":
    ...
    new_dir_path_recursive = f"remote_save_result/Rastrigin_2d_total{now:%m:%d:%H:%M:%S}"
        ...
        for E_var in tqdm(noise_var_list):
            for w_init_0 in w_init_list_0:
                for w_init_1 in w_init_list_1:
                    ...
                    title = f"RVSGD Rastrigin trial = {trial_num} noise = {noise_type} D = {d}_sample_num{n} noise var = {E_var}　w_init = {w_init} f_noise{noise_type_f} f_noise_E{f_E_var}"
