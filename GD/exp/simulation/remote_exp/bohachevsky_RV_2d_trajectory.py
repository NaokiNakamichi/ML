...
if __name__ == "__main__":
    ...
    new_dir_path_recursive = f"remote_save_result/Bohachevsky_2d_trajectory{now:%m:%d:%H:%M:%S}"
        ...
        for E_var in tqdm(noise_var_list):
            for w_init_0 in w_init_list_0:
                for w_init_1 in w_init_list_1:
                    result_to_pickle = {}
                    son = loss.Bohachevsky(d=d, noise_type=noise_type, E_var=E_var, f_E_var=f_E_var,
                    ...
                    title = f"RVSGD_Bohachevsky_trajectory_noise_{noise_type}_D_{d}_sample_num{n}_noise_va_{E_var}_w_init_{w_init_0}_{w_init_1}f_noise{noise_type_f}f_noise_E{f_E_var}.pickle"
